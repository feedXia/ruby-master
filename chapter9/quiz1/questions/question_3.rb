# To complete this challenge, focus on one method at a time.
#
...
def run_game
  ...
  while true
    my_move = move
    if my_move == "hit"
      my_hand.push(random_card)
      my_score = score(my_hand)
      puts "Score so far: #{score(my_hand)}"
    elsif my_move == "stick"
      if my_score > 21
        puts "You busted with: #{score(my_hand)}"
      else
        puts "You scored: #{score(my_hand)}"
      end
      return
    end
  end
